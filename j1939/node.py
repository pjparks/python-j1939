...
class Node(Listener):
    ...
    def on_message_received(self, pdu):
        if pdu.pgn == PGN_AC_ADDRESS_CLAIMED:
            logging.debug('got PGN_AC_ADDRESS_CLAIMED pdu')
            ...
            logging.debug('got PGN_AC_COMMANDED_ADDRESS pdu')
            node_name = NodeName()
            ...
            logging.debug('got PGN_REQUEST_FOR_PGN pdu')
    ...
    def send_parameter_group(self, pgn, data, destination_device_name=None):
        ...
        """
        logging.debug('send_parameter_group:')
        ...
        if self.known_node_addresses[self.node_name.value] not in (ADDRESS_UNCLAIMED, DESTINATION_ADDRESS_NULL):
            logging.debug('send_parameter_group: claimed address')
            pdu = self._pdu_type()
